...
const Checkout = observer(() => {
    ...
    const [checkout, setCheckout] = useState({
        ...
        point: (user.isAuth) ? user.user.point : 0,
        time: 1,
        timevalue: time,
        datevalue: dateNow,
        payment: 'card',
    ...
    const submit = async (e) => {
        e.preventDefault()
        setSendLoading(true)
        if (checkout.delivery == 2 && !checkout.address) {
        ...
        if (data && data.data && data.data.Success && data.data.PaymentURL) {
            setSendOrder({ status: 2, link: data.data.PaymentURL })
            cart.removeAllCart()
    ...
    const change = (e) => {
        if (e.target.name === 'address') {
            ...
            let streetId = streets.rows.find(street => street.title === e.target.value && street.apiId)
            if (streetId && streetId.apiId) {
                ...
                (timeNow.getHours() === 21 || timeNow.getHours() < 10) ?
                    <section className="mb-8">
                        <div className="container">
                            <div className="row justify-content-center">
                                <div className="col-md-7 col-lg-6 col-xl-5">
                                    <h2 className="text-center">Мы работаем с 10:00 до 21:00</h2>
                                                ...
                                                <div className="sec-font">
                                                    <div>
                                                        <div className="d-flex align-items-center mb-3">
                                                            ...
                                                            <label for="card" className="ms-2">Банковской картой курьеру</label>
                                                        </div>
                                                        <div className="d-flex align-items-center mb-3">
                                                        ...
                                                        <div className="d-flex align-items-center mb-3">
                                                            <input type="radio" name="payment" onClick={change} value="ecard" id="ecard" />
                                                            <label for="ecard" className="ms-2">Онлайн оплата</label>
                                                        </div>
                                                    </div>
                                                </div>
                                            </fieldset>
